...
class CoCoDataset_aug_update(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = Image.open('/media/pc3048/8e92fcae-9dcb-48b7-818b-82c5f8ed050e/aidata/lvxiaotian/Structure-CLIP/data/' + img_path)
        """if self.transform is not None:
            img = self.transform(img)"""
        ...
        for pair in pairs:
            caption += pair[1] + " is " + pair[0] + ". "
        caption = caption[:-1]  
        #text = tokenize(text,  truncate=True)
        text = tokenize(caption,  truncate=True)
        head_word_list = list()
        relation_word_list = list()
        tail_word_list = list()
